...
const API_BASE = 'https://llmapi.inferia.ai/api5000/admin';
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      if (!statsRes.ok || !healthRes.ok || !balancesRes.ok) {
        throw new Error(
          `Fetch failed (${statsRes.status}/${healthRes.status}/${balancesRes.status})`
        );
      }
        ...
        <div className="job-sections">
          ...
          <div className="job-section">
            ...
            <div className="job-grid">
              <div>
                <strong>State:</strong>{' '}
                <span className={`job-state state-${jobDetails.state}`}>
                  {jobDetails.state || 'Unknown'}
                </span>
              </div>
              <div>
                <strong>Type:</strong> {jobDetails.type || 'Unknown'}
              </div>
              <div>
                <strong>Price:</strong> {jobDetails.price || 0} NOS
              </div>
              <div>
                <strong>USD/Hour:</strong> ${jobDetails.usdRewardPerHour?.toFixed(6) || '0.000000'}
              </div>
            </div>
          </div>
          ...
          <div className="job-section">
            ...
            <div className="job-grid">
              <div>
                ...
                {jobDetails.timeStart
                  ? new Date(jobDetails.timeStart * 1000).toLocaleString()
                  : '-'}
              </div>
              <div>
                <strong>Listed At:</strong>{' '}
                {jobDetails.listedAt ? new Date(jobDetails.listedAt * 1000).toLocaleString() : '-'}
              </div>
              <div>
              ...
              <div>
                ...
                {jobDetails.timeEnd
                  ? new Date(jobDetails.timeEnd * 1000).toLocaleString()
                  : 'Not ended'}
              </div>
              <div>
                <strong>Benchmark Processed:</strong>{' '}
                {jobDetails.benchmarkProcessedAt
                  ? new Date(jobDetails.benchmarkProcessedAt * 1000).toLocaleString()
                  : 'Not processed'}
              </div>
            </div>
          </div>
          ...
          {jobDetails.jobDefinition?.ops?.[0]?.args && (
            <div className="job-section">
              ...
              <div className="job-grid">
                <div>
                  <strong>Image:</strong> <code>{jobDetails.jobDefinition.ops[0].args.image}</code>
                </div>
                <div>
                  <strong>GPU:</strong>{' '}
                  {jobDetails.jobDefinition.ops[0].args.gpu ? '✅ Yes' : '❌ No'}
                </div>
                <div>
                  <strong>Ports:</strong>{' '}
                  {jobDetails.jobDefinition.ops[0].args.expose?.length > 0
          ...
          {jobDetails.jobDefinition?.meta?.system_requirements && (
            <div className="job-section">
              ...
              <div className="job-grid">
                <div>
                  <strong>CUDA Version:</strong>{' '}
                  {jobDetails.jobDefinition.meta.system_requirements.required_cuda?.join(', ') ||
                    'Not specified'}
                </div>
                <div>
                  <strong>Trigger:</strong> {jobDetails.jobDefinition.meta.trigger || 'Unknown'}
                </div>
              </div>
            </div>
          )}
          ...
          <div className="job-section">
            ...
            <div className="job-grid">
              <div>
                <strong>Type:</strong> {jobDetails.jobDefinition?.type || 'Unknown'}
              </div>
              <div>
                <strong>Version:</strong> {jobDetails.jobDefinition?.version || 'Unknown'}
              </div>
              <div>
                <strong>Operation ID:</strong> {jobDetails.jobDefinition?.ops?.[0]?.id || 'Unknown'}
              </div>
              <div>
                <strong>Operation Type:</strong>{' '}
                {jobDetails.jobDefinition?.ops?.[0]?.type || 'Unknown'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div key={id} className={`server-card ${status} ${isExpanded ? 'expanded' : ''}`}>
        <div className="server-header" onClick={() => toggleServerExpansion(id)}>
          ...
          <div className="server-metrics">
            <span className="concurrent">
              {server.concurrent || 0}/{server.maxConcurrent || 0}
            </span>
        ...
        <div className="server-body">
          <div className="server-basic-info">
            <div>
              <strong>Model:</strong>{' '}
              {Array.isArray(server.model) ? server.model.join(', ') : server.model || 'Unknown'}
            </div>
            <div>
              <strong>Region:</strong> {server.region || 'Unknown'}
            </div>
            <div>
              <strong>Capabilities:</strong> {server.capabilities?.join(', ') || 'None'}
            </div>
          ...
          <div className="server-performance">
            <div>
              ...
              {server.performance?.avgResponseTime
                ? `${Math.round(server.performance.avgResponseTime)}ms`
                : '-'}
            </div>
            <div>
              <strong>P95 Response:</strong>{' '}
              {server.performance?.p95ResponseTime
                ? `${server.performance.p95ResponseTime}ms`
                : '-'}
            </div>
            <div>
              <strong>Throughput:</strong>{' '}
              {server.performance?.throughput
                ? `${server.performance.throughput.toFixed(1)}/s`
                : '-'}
            </div>
            <div>
              <strong>Error Rate:</strong>{' '}
              {server.performance?.errorRate
                ? `${(server.performance.errorRate * 100).toFixed(1)}%`
                : '-'}
            </div>
          </div>
          ...
          {isExpanded && server.circuitBreaker && (
            <div className="circuit-breaker-info">
              ...
              <div className="cb-grid">
                <div>
                  <strong>State:</strong> {server.circuitBreaker.state}
                </div>
                <div>
                  <strong>Failures:</strong> {server.circuitBreaker.failureCount}
                </div>
                <div>
                  <strong>Next Attempt:</strong>{' '}
                  {server.circuitBreaker.nextAttempt
                    ? new Date(server.circuitBreaker.nextAttempt).toLocaleString()
                    : 'Immediate'}
                </div>
              </div>
            </div>
    ...
    <div className="admin-dashboard">
      ...
      <div className="cards">
        <div className="card">
          <h3>Load Balancer</h3>
          <div className="row">
            <strong>Total Servers:</strong> {stats?.loadBalancer?.totalServers ?? '-'}
          </div>
          <div className="row">
            <strong>Healthy Servers:</strong> {stats?.loadBalancer?.healthyServers ?? '-'}
          </div>
          <div className="row">
            <strong>Total Concurrent:</strong> {stats?.loadBalancer?.totalConcurrent ?? '-'}
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="row">
            <strong>Memory (RSS):</strong> {formatBytes(stats?.system?.memory?.rss)}
          </div>
          <div className="row">
        ...
        <div className="card">
          ...
          <div className="row">
            ...
            <span className="public-key" title={balances?.publicKey}>
              {balances?.publicKey
                ? `${balances.publicKey.slice(0, 8)}...${balances.publicKey.slice(-8)}`
                : '-'}
            </span>
          </div>
          <div className="row">
            <strong>SOL:</strong> {formatTokenBalance(balances?.sol)}
          </div>
          <div className="row">
            <strong>USDC:</strong> {formatTokenBalance(balances?.usdc)}
          </div>
          <div className="row">
            <strong>NOS:</strong> {formatTokenBalance(balances?.nos)}
          </div>
        </div>
      </div>
      ...
      <section style={{ marginTop: 18 }}>
        ...
        <pre className="raw">{JSON.stringify({ health, stats, balances }, null, 2)}</pre>
      </section>
    </div>
  );
}
